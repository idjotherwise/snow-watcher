...
async def get_report_async(city: str, state: Optional[str], country: str, units: str) -> dict:
    ...
    data = resp.json()
    forecast = {'weather': data['weather'][0]['main'], 'description': data['weather'][0]['description'].capitalize(), **data['main']}
...
def validate_units(city: str, state: Optional[str], country: Optional[str], units: str) -> Tuple[str, Optional[str], str, str]:
    ...
    if len(country) != 2:
        error = f'Invalid country: {country}. It must be a two letter abbreviation such as US or UK'
